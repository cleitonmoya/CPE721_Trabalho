...
H = [3, 4, 5, 6, 7];                                % número de neurônios na cadama oculta
                ...
                for j = 1:length(P2)
                    ...
                    for k=1:K
                        ...
                        if ~strcmp(ini, 'default')
                            net.iw{1} = inicializaPesos(h,36,h,ini);
...
% Melhor modelo
subplot(2,1,1)
subtitle('Melhor modelo: RPROP, H=6, inic. tipo 1')
hold on
...
y_vl = y(:,tr_ws.valInd);
g = net(X_vl); % predição
[error, ~, ~, ~] = confusion(heaviside(y_vl),heaviside(g));
acc = 1-error;
fprintf('Acurácia final no conjunto de teste: %f\n',acc)

figure() 
cm = confusionchart(y_vl, sat(g));
